...
class fukui:
    ...
    def get_data(self):
        ...
        for file_name in self.data.keys():
            ...
            if neutral_data != None and  oxidized_data != None and reduced_data != None:
                self.args.log.write(f"Reading information for FUKUI data from {file_name}\n")
                file_data[file_name]['neutral']['atomcharges']['natural'] = neutral_data.atomcharges['natural']
                file_data[file_name]['neutral']['atomcharges']['cm5'] = neutral_data.atomcharges['cm5']
                file_data[file_name]['neutral']['atomcharges']['mulliken'] = neutral_data.atomcharges['mulliken']
                file_data[file_name]['neutral']['atomcharges']['hirsfeld'] = neutral_data.atomcharges['hirsfeld']

                file_data[file_name]['oxidized']['atomcharges']['natural'] = oxidized_data.atomcharges['natural']
                file_data[file_name]['oxidized']['atomcharges']['cm5'] = oxidized_data.atomcharges['cm5']
                file_data[file_name]['oxidized']['atomcharges']['mulliken'] = oxidized_data.atomcharges['mulliken']
                file_data[file_name]['oxidized']['atomcharges']['hirsfeld'] = oxidized_data.atomcharges['hirsfeld']

                file_data[file_name]['neutral']['atomcharges']['natural'] = reduced_data.atomcharges['natural']
                file_data[file_name]['neutral']['atomcharges']['cm5'] = reduced_data.atomcharges['cm5']
                file_data[file_name]['neutral']['atomcharges']['mulliken'] = reduced_data.atomcharges['mulliken']
                file_data[file_name]['neutral']['atomcharges']['hirsfeld'] = reduced_data.atomcharges['hirsfeld']
            else:
                self.args.log.write(f"Skipping file {file_name} as one either neutral, oxidized or reduced didnt exist\n")
    ...
    def parse_cc_data(self, file_name, file):
        ...
        try: #hirsfeld & cm5
            ...
            lines = open(file,"r").readlines()
            for i,line in enumerate(lines):
                if line.find("Hirshfeld charges, spin densities, dipoles, and CM5 charges using IRadAn=      5:") > -1: start_npop = i+2
            if start_npop != None:
                hers_charges = []
                cm5_charges = []
                end_npop = start_npop + len(cc_data.atomnos)
