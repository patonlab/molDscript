    ...
    def get_data(self):
        ...
        for file_name in self.data.keys():
            ...
            if ie_data != None and ea_data != None:
                self.args.log.write(f"Reading information for IE & EA data from {file_name}\n")
                file_data[file_name]['ie']['E']= ie_data.scfenergies[-1]
                file_data[file_name]['ea']['E'] = ea_data.scfenergies[-1]
            else:
                self.args.log.write(f"Skipping file {file_name} as one either IE or EA didnt exist\n")
