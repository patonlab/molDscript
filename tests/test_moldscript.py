...
@pytest.mark.parametrize("opt_path, program, species, energy, enthalpy, gibbs_energy, smiles", [
    ('arbr/opt', 'gaussian', 'arbr31_wb97xd', -2996.631169953559, -2996.486642, -2996.532564, '[H]c1c([H])c2c(c([H])c1Br)C([H])([H])C([H])([H])C2=O'),
])

def test_opt(opt_path, program, species, energy, enthalpy, gibbs_energy, smiles):
...
@pytest.mark.parametrize("opt_path, nmr_path, program, species, shifts", [
    ('arbr/opt', 'arbr/nmr', 'gaussian', 'arbr31_wb97xd', [-257.5005, -20.2833, 149.8784, 159.2040, 24.3340, 
     52.8746, 34.6443, 2046.8050, 52.1611, 57.5798, 45.7539, 29.2925, 29.2924, 28.7646, 28.7645, 24.0512, 24.1362, 23.8194]),
])

def test_nmr(opt_path, nmr_path, program, species, shifts):
    path = datapath(opt_path)
    data_dicts = {}
    ...
    nmr_read = files("nmr", path, data_dicts, program = program)
    nmr_data = nmr(nmr_read.file_data, data_dicts, program = program)
...
@pytest.mark.parametrize("opt_path, nbo_path, program, species, natural_charges, bond_orders", [
    ('arbr/opt', 'arbr/popn', 'gaussian', 'arbr31_wb97xd', [-0.52472, 0.57494, -0.50777, -0.42749, 
...
def test_nbo(opt_path, nbo_path, program, species, natural_charges, bond_orders):
    path = datapath(opt_path)
    data_dicts = {}
    ...
    nbo_read = files("nbo", path, data_dicts, program = program)
    nbo_data = nbo(nbo_read.file_data, data_dicts, program = program)
...
@pytest.mark.parametrize("opt_path, program, species, dipole, homo, lumo, quadrupole_moment_trace", [
    ('arbr/opt', 'gaussian', 'arbr31_wb97xd', 2.536, -0.33291, -0.00473, -234.10379),
])

def test_fmo(opt_path, program, species, dipole, homo, lumo, quadrupole_moment_trace):
    path = datapath(opt_path)
    data_dicts = {}
    ...
    fmo_data = fmo(fmo_read.file_data, data_dicts, program = program)
...
@pytest.mark.parametrize("opt_path, program, species, apt_charges", [
    ('arbr/opt', 'gaussian', 'arbr31_wb97xd', [-0.813669,  1.084436, -0.094902,  0.06463 ,  0.119114, -0.205275,
        0.450111, -0.26189 , -0.244025,  0.105473, -0.406945,  0.018934,
        0.018939, -0.012211, -0.012213,  0.061321,  0.065093,  0.063078]),
])

def test_charges(opt_path, program, species, apt_charges):
    path = datapath(opt_path)
    data_dicts = {}
    ...
    chg_data = charges(chg_read.file_data, data_dicts, program = program)
...
@pytest.mark.parametrize("opt_path, fukui_neutral_path, fukui_oxidized_path, fukui_reduced_path, program, species, oxidized_charges, reduced_charges", [
    ('arbr/opt', 'arbr/popn', 'arbr/fukui_ox', 'arbr/fukui_red', 'gaussian', 'arbr31_wb97xd', [-0.07481, 0.55092, -0.42669, -0.43292, 0.09900, -0.24454, 
...
def test_fukui(opt_path, fukui_neutral_path, fukui_oxidized_path, fukui_reduced_path, program, species, oxidized_charges, reduced_charges):
    path = datapath(opt_path)
    data_dicts = {}
    ...
    fukui_read = files(
                ...
                program = program)
    fukui_data = fukui(fukui_read.file_data, data_dicts, program = program)
